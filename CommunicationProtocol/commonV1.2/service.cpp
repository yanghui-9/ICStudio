...
int32_t Device::AnalysisFrame(Frame &recvF)
{
    ...
    case conditionRead:
    {
       for(uint32_t i = 0;i < recvF.readList.size();i++)
       {
           ...
           //开始解析.
           ret = m_DataAreaDeal.SetDataFromAddr(recvF.readList.at(i),recvF.GetRecvBuff()+recvF.readList.at(i).beginIndex);
       }
    }
        break;
    case write:
...
int32_t Device::GetLastData(AddrInfoForRW &addr)
{
    ...
    if(Protocol::string != addr.dataType)
    {
        ...
        bufv.resize(len/8);
        //一次性获取数据.
        m_DataAreaDeal.GetDataFromAddr(addr.reg,index,len,bufv.data());
        if(1 != addr.len)
        {//字.
            //开始处理一个个数据.
            //处理字节序.
    ...
    {//string
      ...
      #endif
      if(-1 == m_DataAreaDeal.GetDataFromAddr(addr.reg,addr.index,addr.len,sData.data()) )
      {
...
int32_t Device::SetLastData(Protocol::AddrInfoForRW &addr)
{
    ...
    {//非只写类型才有地址缓存.
        ...
        if(Protocol::string != addr.dataType)
        {
            ...
            //数据写入.
            m_DataAreaDeal.SetDataFromAddr(addrinfo,bufv.data());
        }
        else
        {//string
            std::string sData;
            ...
            sData.resize(addr.len/8);
            if(-1 == m_DataAreaDeal.SetDataFromAddr(addrinfo,sData.data()))
            {
                ret = Protocol_Rtn_Fail;
            }
        }
