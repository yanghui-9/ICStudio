...
//返回值定义
#define Protocol_Process_Null (-1)
...
struct RegInfo;
using DataVariant = std::variant<char, unsigned char,short,unsigned short,uint32_t,int32_t,float,uint64_t,int64_t,double,std::string>;
using DataVector = std::vector<DataVariant>;
//using DataCallback = void (*)(const AddrInfo &,int32_t);
//using DataWCallback = void (*)(const AddrInfoForRW &,int32_t);
using DataCallback = std::function<void (const AddrInfo &,int32_t)>;
...
struct AddrInfo
{
    std::string linkName;//连接名称
    std::string reg;     //地址模式下-寄存器，标签模式下-标签名称
    std::string sIndex;  //字符串格式地址
    ...
    bool operator == (const AddrInfo & info) const
    {
        if(reg == info.reg)
        {
            if((index <= info.index && index+len > info.index) || (info.index <= index && info.index+info.len > index))
            {//地址有交叉
                return true;
            }
        }
...
struct AddrInfoForRW
{
    ...
    uint64_t num = 1;              //地址模式下-数据个数，协议标签模式下无效
    DataWCallback callback = nullptr;//回调
    Protocol::DataVector valueVec;//值
    bool operator == (const AddrInfoForRW & info) const
    {
        if(reg == info.reg && dataType == info.dataType &&
                index == info.index && num == info.num)
        {
            return true;
        }
        return false;
...
struct Tag_Info
{
    ...
    Tag_Info(const std::string &_name,
             ...
             const Protocol_DataType &_dataType){
       name = _name;
       dataType = _dataType;
       rwType = _rwType;
    }
...
struct RegInfo
{
    ...
    RW_Type  rwFlag;      //读写类型
    uint32_t unitBitNum; //一个整数地址单位对应位的个数
    ...
    {reg = r;format = f;indexMax = iMax;indexMin = iMin;rwFlag = rwF;unitBitNum = uBitNum;dataType = ubit16;}
    RegInfo(const std::string r,RW_Type rwF,Protocol_DataType dType)
    {reg = r;indexMax = 0;indexMin = 0;rwFlag = rwF;unitBitNum = 0;dataType = dType;}
    RegInfo(){}
};
...
using P_SetCycReadAddrinfo = int32_t (*)(const std::vector<Protocol::AddrInfoForRW> &addrVec,Protocol::Add_Type);
using P_SetRegValueChangeCallback = int32_t (*)(const std::vector<Protocol::AddrInfoForRW> &addrVec,Protocol::Add_Type, Protocol::DataCallback callback);
using P_SetValue =  int32_t (*)(Protocol::AddrInfoForRW &, const Protocol::Write_Opt_Type &);
using P_GetValue = int32_t (*)(Protocol::AddrInfoForRW &,const Protocol::Read_Opt_Type &);
...
using P_CheckAddr = int32_t (*)(Protocol::AddrInfo &info,bool);
