...
public:
    ...
    ICommunication(){}
    virtual ~ICommunication(){ReadReady_Bind(nullptr,true);Connnect_Bind(nullptr,true);Disconnect_Bind(nullptr,true);}
    /**
     * @brief   发送数据
     * @note
     ...
     * @return  成功0，失败-1
     */
    virtual int32_t OpenComm(int setnonblock = 0,int Timo = 500) = 0;
    ...
    /**
     * @brief    三种类型触发回调，支持普通函数指针和绑定指定对象的成员函数
     * @return   成功返回0，失败-1
     */
    CB_CLASS_BIND(Connnect, __connection00 , m_connect_slots ,(const std::string,int32_t,int32_t&))

    CB_BIND(Connnect,__connection01, 0, m_connect_slots)

    CB_CLASS_BIND(ReadReady, __connection10 , m_readReady_slots, (int32_t,int32_t))

    CB_BIND(ReadReady,__connection11, 1, m_readReady_slots)

    CB_CLASS_BIND(Disconnect, __connection20 , m_disconnect_slots ,(int32_t))

    CB_BIND(Disconnect,__connection21, 2, m_disconnect_slots)
protected:
    __connection_base0 *m_connect_slots = nullptr;
    __connection_base1 *m_readReady_slots = nullptr;
    __connection_base2 *m_disconnect_slots = nullptr;
};
 ...
 *          8888，服务端只有端口号
 */
_API ICommunication * CreateComm(ICommunication::Communication_Type type, const std::string &sPar);
 ...
 * @return  成功返回0，失败-1
 */
_API int CloseCommunication(ICommunication *pCommunication);
